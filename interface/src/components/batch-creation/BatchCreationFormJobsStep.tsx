...
function BatchCreationFormJobsStep(props: {
  ...
  return (
    <>
      ...
      <TextField label="Identifying variable" name="script.identifyingVariable" required />
      ...
      <Field name="script.job" textareaId="job" component={CodeEditor} label="Job contents" />
    </>
  );
}

BatchCreationFormJobsStep.validate = (values: FormValues) => {
  if (!values.script?.identifyingVariable?.trim().length) {
    return { script: { identifyingVariable: "Please enter a value" } };
  }
  return {};
};

const toExport = BatchCreationFormJobsStep as BatchCreationStep;
