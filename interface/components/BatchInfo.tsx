...
function BatchInfo({ batch }: { batch: BatchMetadata }) {
  return (
    <div>
      <h1>{batch.name}</h1>
      {/* <ProgressBar
      progress={
        batch.statusSummary.success ? (batch.statusSummary.success / batch.jobs.length) * 100 : 0
      }
    /> */}
      {batch.jobs && (
        <div>
          ...
          <h3>Job Summary</h3>
          <ul>
            {batch.statusSummary.success && <li>Success: {batch.statusSummary.success}</li>}
